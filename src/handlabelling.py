...
# if do with classification, make sure to end with button 'q' (do not interrupt program with Ctrl+C or similar !)
...
BASE_PATH = '../datasets/dataset_downsampled_devel/DischmaCams/'
STATCAM_LST = [
    'Buelenberg/1',
    'Buelenberg/2',
    'Giementaelli/1',
...
CAMSTAT = 'Buelenberg/2'  # to choose (one of STATCAM_LST)
    ...
    for file in filelist:
        ...
        if (file[0:4] == '2020' or file[0:4] == '2021') and file.endswith(ext):  # only label files of 2020/2021 
            ...
            if key == ord('f'):
                line = line + '1\n'
                txtfile.write(line)
                #write_to_file(string=line, textfile=txtfile)
...
print('everything from years 2020 and 2021 classified, thanks :) ')